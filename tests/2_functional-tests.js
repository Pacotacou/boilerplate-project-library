...
suite('Functional Tests', function() {
  ...
  */
  ...
  suite('Routing tests', function() {
    ...
    let testId; // Will store a valid _id for later tests
      ...
      test('Test POST /api/books with title', function(done) {
        chai.request(server)
          ...
          .end(function(err, res) {
            ...
            testId = res.body._id; // Store the _id for later use
            done();
          });
      });
    ...
    suite('GET /api/books/[id] => book object with [id]', function() {
      ...
      test('Test GET /api/books/[id] with id not in db', function(done) {
        chai.request(server)
          .get('/api/books/64dcd08f1111111111111111') // Random invalid ObjectId
          .end(function(err, res) {
            assert.equal(res.status, 200);
    ...
    suite('POST /api/books/[id] => add comment/expect book object with id', function() {
      ...
      test('Test POST /api/books/[id] with comment, id not in db', function(done) {
        chai.request(server)
          .post('/api/books/64dcd08f1111111111111111') // Random invalid ObjectId
          .send({ comment: 'Test Comment' })
          .end(function(err, res) {
            assert.equal(res.status, 200);
            assert.equal(res.text, 'no book exists');
    ...
    suite('DELETE /api/books/[id] => delete book object id', function() {
      ...
      test('Test DELETE /api/books/[id] with id not in db', function(done) {
        chai.request(server)
          .delete('/api/books/64dcd08f1111111111111111') // Random invalid ObjectId
          .end(function(err, res) {
            assert.equal(res.status, 200);
            assert.equal(res.text, 'no book exists');
            done();
